...
class Camera(object):
    def __init__(self):
        ...
        #Variables parte grafica
            ...
            # self.line2, = self.ax2.plot(range(38),self.d_blinks,'b-')
    ...
    def plotting(self,x1):
        ...
        # self.d_blinks = numpy.diff(self.blinks,2)
        #
        ...
        # self.line2.set_ydata(self.d_blinks)
    ...
    def main(self):
        ...
        if not rects:
            ...
            if (self.eyes_open + self.eyes_closed) > 500:
            ...
            cv2.putText(self.frames,'Num. Parpadeos:{}' .format(self.total), (15,20),cv2.FONT_HERSHEY_SIMPLEX,0.65,(255, 0, 0),1)
            cv2.putText(self.frames,'Num. Bostezos:{}' .format(self.cont_yawn), (15,40),cv2.FONT_HERSHEY_SIMPLEX,0.65,(255, 0, 0),1)
            #cv2.putText(self.frames,'ER: {}' .format(er), (300,15),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0, 0, 255),1)
            cv2.putText(self.frames,'HR:{}' .format(self.window_hr), (15,460),cv2.FONT_HERSHEY_SIMPLEX,0.65,(255, 255, 255),1)
            cv2.putText(self.frames,'PITCH:{}' .format(angle_pitch), (515,420),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255, 255, 255),1)
            cv2.putText(self.frames,'YAW:{}' .format(yaw), (515,440),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255, 255, 255),1)
            cv2.putText(self.frames,'ROLL:{}' .format(roll), (515,460),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255, 255, 255),1)
...
def main():
    ...
    while not rospy.is_shutdown():
        if enable_img is True and enable_depth is True:
            #print("Entre")
            cam.main()
    #rospy.sleep(0.01)
    rospy.spin()
